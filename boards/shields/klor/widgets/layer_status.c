...
static void set_layer_symbol(lv_obj_t *label, const struct layer_status_state *state) {
    if (state->label == NULL) {
        char text[20] = {};

        sprintf(text, "#0000ff LAYER: %i#", state->index);

        lv_label_set_text(label, text);
    } else {
        char text[19] = {};

        snprintf(text, sizeof(text), "#0000ff %s#", state->label);
...
static void layer_status_update_cb(struct layer_status_state state) {
    struct zmk_layer_status_widget *widget;
...
// layer_status_update_cb, layer_status_get_widget)
